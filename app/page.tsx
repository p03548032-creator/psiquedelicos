...
export default function HomePage() {
  ...
  const featuredArticles = articlesFull.slice(0, 3);
    ...
    <>
      ...
      <section className="relative py-24 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            <span className="text-sm uppercase tracking-[0.3em] text-white/25 block mb-4">Explora el portal</span>
            <h2 className="text-3xl md:text-4xl font-black gradient-text">Todo lo que necesitas saber</h2>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {sections.map(s => (
              <Link key={s.href} href={s.href}
                className="glass-sacred rounded-2xl p-6 hover:scale-[1.03] transition-all duration-300 group relative overflow-hidden">
      ...
      <section className="py-20 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-end justify-between mb-10">
            <div>
              <span className="text-sm uppercase tracking-[0.3em] text-psyche-cyan/60 block mb-3">Investigación</span>
              <h2 className="text-3xl font-black gradient-text-cool">Últimos artículos</h2>
            </div>
            <Link href="/investigacion" className="vesica-btn px-4 py-2 text-sm text-white/50 hover:text-white transition hidden sm:flex items-center gap-2">
              Ver todos <span>→</span>
            </Link>
          </div>
          <div className="space-y-3">
            {featuredArticles.map(a => (
              <Link key={a.id} href={`/articulo/${a.id}`}
                ...
                <span className="text-white/15 group-hover:text-white/40 transition flex-shrink-0">→</span>
              </Link>
            ))}
          </div>
          <div className="flex justify-center mt-6 sm:hidden">
            <Link href="/investigacion" className="vesica-btn px-5 py-2.5 text-sm text-white/50">Ver todos los artículos →</Link>
          </div>
        </div>
      </section>
